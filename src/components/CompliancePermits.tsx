...
import { ShieldCheck, FileCheck, Zap, Building2, AlertTriangle, Search, FileText, Hammer, CheckCircle2, ArrowRight, Flame, ClipboardCheck } from 'lucide-react';

export default function CompliancePermits() {
  const { t } = useTranslation();
  const p = 'compliance_reno_page';
  const whatsappLink = `https://wa.me/6598333085?text=Hi%20ID%20Work%20Studio,%20I%20need%20help%20with%20commercial%20renovation%20permits.`;
  ...
  const relatedKeys = ["office","retail","shophouse","reinstatement"];
  const relatedLinks = ["/commercial/office-renovation","/commercial/retail-mall-renovation","/commercial/shophouse-renovation","/commercial/reinstatement"];

  const pillars = [
    { step:"01", agency:t(`${p}.pillars.p1.agency`), title:t(`${p}.pillars.p1.title`), subtitle:t(`${p}.pillars.p1.subtitle`), body:t(`${p}.pillars.p1.body`), facts:[1,2,3,4,5].map(n=>t(`${p}.pillars.p1.f${n}`)), icon:<Building2 size={32} strokeWidth={1.5}/> },
    { step:"02", agency:t(`${p}.pillars.p2.agency`), title:t(`${p}.pillars.p2.title`), subtitle:t(`${p}.pillars.p2.subtitle`), body:t(`${p}.pillars.p2.body`), facts:[1,2,3,4,5].map(n=>t(`${p}.pillars.p2.f${n}`)), icon:<ClipboardCheck size={32} strokeWidth={1.5}/> },
    { step:"03", agency:t(`${p}.pillars.p3.agency`), title:t(`${p}.pillars.p3.title`), subtitle:t(`${p}.pillars.p3.subtitle`), body:t(`${p}.pillars.p3.body`), facts:[1,2,3,4,5].map(n=>t(`${p}.pillars.p3.f${n}`)), icon:<Flame size={32} strokeWidth={1.5}/> },
    { step:"04", agency:t(`${p}.pillars.p4.agency`), title:t(`${p}.pillars.p4.title`), subtitle:t(`${p}.pillars.p4.subtitle`), body:t(`${p}.pillars.p4.body`), facts:[1,2,3,4,5].map(n=>t(`${p}.pillars.p4.f${n}`)), icon:<Zap size={32} strokeWidth={1.5}/> },
  ];
      ...
      <div className="bg-off-white min-h-screen font-sans selection:bg-gold selection:text-dark-charcoal">
        ...
        <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-dark-charcoal pt-24 md:pt-32 lg:pt-40 pb-20 md:pb-32 text-white">
          <div className="absolute inset-0 z-0">
            <img src="/gallery/commercial-expertise.png" alt="Commercial Renovation Permits Compliance Singapore" className="w-full h-full object-cover opacity-40" fetchpriority="high" decoding="async"/>
          ...
          <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.8,delay:0.5}} className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
              {[<Building2 size={20}/>,<FileCheck size={20}/>,<Flame size={20}/>,<Zap size={20}/>].map((icon,i)=>(
        ...
        <section className="py-24 bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-3xl md:text-5xl mb-4 flex flex-col items-center">
              <span className="font-serif italic text-dark-charcoal/60 text-2xl mb-2">{t(`${p}.intro.serif`)}</span>
              <span className="font-sans font-bold text-dark-charcoal uppercase tracking-wider">{t(`${p}.intro.bold`)}</span>
            </h2>
            <div className="w-24 h-1 bg-gold mx-auto mt-6 mb-12"></div>
            <p className="text-gray-600 text-lg leading-relaxed mb-6">{t(`${p}.intro.p1`)}</p>
            <p className="text-gray-600 leading-relaxed">{t(`${p}.intro.p2`)}</p>
          </div>
        </section>
        ...
        <section className="py-24 bg-off-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-5xl mb-4 flex flex-col items-center">
                <span className="font-serif italic text-dark-charcoal/60 text-2xl mb-2">{t(`${p}.pillars.serif`)}</span>
                <span className="font-sans font-bold text-dark-charcoal uppercase tracking-wider">{t(`${p}.pillars.bold`)}</span>
              </h2>
              <div className="w-24 h-1 bg-gold mx-auto mt-6"></div>
            </div>
            <div className="space-y-12">
              {pillars.map((pillar)=>(
                <motion.div key={pillar.step} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="bg-white rounded-2xl shadow-sm border-l-4 border-gold overflow-hidden">
                  <div className="flex items-start gap-6 p-8 border-b border-gray-50">
                    <div className="flex-shrink-0 w-14 h-14 bg-gold/10 rounded-xl flex items-center justify-center text-gold">{pillar.icon}</div>
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-xs font-bold text-gold uppercase tracking-[0.2em]">{t(`${p}.pillars.step_label`)} {pillar.step}</span>
                        <span className="text-xs text-gray-300">·</span>
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em]">{pillar.agency}</span>
                      </div>
                      <h3 className="text-xl md:text-2xl font-bold text-dark-charcoal font-sans">{pillar.title}</h3>
                      <p className="text-sm text-gold/80 mt-1 font-medium italic">{pillar.subtitle}</p>
                    </div>
                  </div>
                  <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-gray-400 mb-4 font-bold">{t(`${p}.pillars.facts_label`)}</p>
                      <ul className="space-y-2">
                        {pillar.facts.map((fact,j)=>(
                          <li key={j} className="flex items-start gap-2 text-sm text-gray-600">
                            <span className="text-gold flex-shrink-0 mt-0.5">·</span>{fact}
                          </li>
                        ))}
                      </ul>
                    </div>
        ...
        <section className="py-24 bg-off-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-6">
              {[{n:1,icon:<Flame className="w-6 h-6 text-gold flex-shrink-0 mt-1"/>},{n:2,icon:<Zap className="w-6 h-6 text-gold flex-shrink-0 mt-1"/>},{n:3,icon:<ShieldCheck className="w-6 h-6 text-gold flex-shrink-0 mt-1"/>},{n:4,icon:<AlertTriangle className="w-6 h-6 text-gold flex-shrink-0 mt-1"/>}].map(({n,icon})=>(
